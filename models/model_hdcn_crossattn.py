...
class Attn(nn.Module):
    ...
    def forward(self, target_node, neighbor):
        ...
        K = self.key_proj(neighbor)       # (batchl_size, seq_len, convolution_dim)
...
class full_module(nn.Module):
    def __init__(self):
        ...
        self.norm_A = nn.LayerNorm(self.dim)
        self.norm_B = nn.LayerNorm(self.dim)
    ...
    def forward(self, x):
        ...
        A_attn = self.atten_A(x_A, x_B)
        B_attn = self.atten_B(x_B, x_A)
        updated_x_A = x_A + A_attn
        updated_x_B = x_B + B_attn
        # LayerNorm expects the normalized dimension last:
        # (batch, channels, time) -> (batch, time, channels)
        updated_x_A = self.norm_A(updated_x_A.permute(0, 2, 1)).permute(0, 2, 1)
        updated_x_B = self.norm_B(updated_x_B.permute(0, 2, 1)).permute(0, 2, 1)
        updated_x_A = updated_x_A.permute(0, 2, 1).view(-1, seq_len, self.dim, 1)
        updated_x_B = updated_x_B.permute(0, 2, 1).view(-1, seq_len, self.dim, 1)
