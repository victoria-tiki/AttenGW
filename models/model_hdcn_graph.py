...
class full_module(nn.Module):
    def __init__(self):
        ...
        self.conv1d=nn.Conv1d(self.node_dim,1, 1) #change to dim for attn, node_dim for orig
    ...
    def forward(self, x):
        ...
        updated_x_A = self.pinsage_A(x_A, x_B).permute(0,2,1).view(-1, 4096, self.node_dim, 1) #change to dim for attn, node_dim for orig
        updated_x_B = self.pinsage_B(x_B, x_A).permute(0,2,1).view(-1, 4096, self.node_dim, 1)#change to dim for attn, node_dim for orig
