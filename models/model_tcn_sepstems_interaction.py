...
class TCNStem(nn.Module):
    ...
    Applied separately to each detector channel with the same weights.
...
class full_module(nn.Module):
    """
    Model F: TCN stem + detector-interaction head.
    ...
    def __init__(self, *args, hidden_channels: int = 32, stem_dilations=None, dropout: float = 0.0, return_logits: bool = False, **kwargs):
        ...
        # A, B, A * B, abs(A - B) -> 4 * hidden_channels
    ...
    def forward(self, x):
        ...
        # Input x: (batch, time, 2)
        # A/B:    (batch, 1, time)
        A = x[:, :, 0].unsqueeze(1)
        B = x[:, :, 1].unsqueeze(1)

        # Shared TCN features:
        # A_feat/B_feat: (batch, hidden_channels, time)
